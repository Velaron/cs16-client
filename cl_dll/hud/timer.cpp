...
int CHudProgressBar::Draw( float flTime )
{
	...
	if( m_szLocalizedHeader && m_szLocalizedHeader[0] )
	{
		...
		DrawUtils::DrawRectangle( ScreenWidth/ 4, ScreenHeight / 2 + DrawUtils::HudFontHeight(), ScreenWidth/2, ScreenHeight/30 );
		FillRGBA( ScreenWidth/4+2, ScreenHeight/2 + DrawUtils::HudFontHeight() + 2, m_fPercent * (ScreenWidth/2-4), ScreenHeight/30-4, 255, 140, 0, 255 );
		return 1;
	}

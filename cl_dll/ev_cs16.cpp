...
void EV_FireAK47( event_args_t *args )
{
	...
	shell = gEngfuncs.pEventAPI->EV_FindModelIndex ("models/gib_skull.mdl");
	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, -right, up, 12, -10, -7 );
	EV_EjectBrass(ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL);
...
void EV_FireAWP( event_args_t *args )
{
	...
	{
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( AWP_SHOOT1 + gEngfuncs.pfnRandomLong(0,3), 2 );
		V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -2, 2 ) );
	}
...
void EV_FireDEAGLE( event_args_t *args )
{
	...
	{
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(DEAGLE_SHOOT1 + gEngfuncs.pfnRandomLong(0,2), 2);
		gHUD.RealSize += 150;
		//V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -2, 2 ) );
	}

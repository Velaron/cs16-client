...
float SetController(void *pmodel, entvars_t *pev, int iController, float flValue)
{
	...
	if (pbonecontroller->type & (STUDIO_XR | STUDIO_YR | STUDIO_ZR))
	{
		...
		{
			if (flValue > 360.0)
				flValue = flValue - (flValue / 360.0) * 360.0;

			else if (flValue < 0.0)
				flValue = flValue + ((flValue / -360.0) + 1) * 360.0;
		}
		else
		{
...
float SetBlending(void *pmodel, entvars_t *pev, int iBlender, float flValue)
{
	...
	int setting = (255.0f * (flValue - pseqdesc->blendstart[iBlender]) / (pseqdesc->blendend[iBlender] - pseqdesc->blendstart[iBlender]));
...
void AngleQuaternionAni(vec_t *angles, vec_t *quaternion)
{
	float sy, cy, sp_, cp;
	float angle;
	float sr, cr;
...
void StudioCalcBoneQuaterion(int frame, float s, mstudiobone_t *pbone, mstudioanim_t *panim, float *adj, float *q)
{
	...
	{
		AngleQuaternionAni(angle1, q1);
      AngleQuaternionAni (angle2, q2);
		QuaternionSlerp(q1, q2, s, q);
	}
	else
      AngleQuaternionAni (angle1, q);
}
...
void SV_StudioSetupBones(struct model_s *pModel, float frame, int sequence, const vec3_t angles, const vec3_t origin, const byte *pcontroller, const byte *pblending,int iBone, const edict_t *pEdict)
{
	...
	if (pEdict != NULL)
	{
		temp_angles[1] = UTIL_GetPlayerGaitYaw(ENTINDEX(const_cast <edict_t *> (pEdict)));
